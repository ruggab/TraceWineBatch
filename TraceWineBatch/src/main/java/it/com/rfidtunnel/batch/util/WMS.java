...
import pkg.controls.CheckBox;
import pkg.entity.Item2;
import pkg.entity.Packages;
...
public class WMS implements Runnable {
	...
	public void run() {
		...
		try {
			...
			WMS_IP = Utility.getProp(CheckBox.conn, "WMSIP");
			WMS_PORT = Integer.parseInt(Utility.getProp(CheckBox.conn, "WMSPORT"));
			SEPARATOR = Utility.getProp(CheckBox.conn, "SEPARATOR");
